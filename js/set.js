...
function setBgImgInit() {
    ...
    switch (bg_img["type"]) {
        case "2":
            $('#bg').attr('src', 'https://dailybing.com/api/v1 ') //必应每日
            break;
        case "3":
...
$(document).ready(function () {
    ...
    $("#wallpaper").on("click", ".set-wallpaper", function () {
        ...
        if (type === "2") {
            $('#wallpaper_text').html("显示必应每日一图，每天更新");
            setBgImg(bg_img);
            iziToast.show({
                message: '壁纸设置成功，刷新生效',
            });
        ...
        if (type === "3") {
            $('#wallpaper_text').html("显示随机风景图，每次刷新后更换");
            setBgImg(bg_img);
            iziToast.show({
                message: '壁纸设置成功，刷新生效',
            });
        ...
        if (type === "4") {
            $('#wallpaper_text').html("显示随机二次元图，每次刷新后更换");
            setBgImg(bg_img);
            iziToast.show({
                message: '壁纸设置成功，刷新生效',
            });
